...
public class TestReaderGZIP {

	public TestReaderGZIP() {
		init();
	}
	...
	// 提取AnchorTag之间的全部内容，进行处理
	// 每读取到一条AnchorTag就重制输出流，将AnchorTag之间未处理的内容写入输出流传递到下一个函数
	...
	public void processHrefAndAnchorText(ByteArrayOutputStream anchorText, ByteArrayOutputStream href,
		...
		if (sHref.length() > 0) {
			if (sHref.startsWith("#")) {
				validHref = false;
			} else if (sHref.toLowerCase().startsWith("javascript")) {
				validHref = false;
			} else if (sHref.toLowerCase().startsWith("mailto:")) {
				validHref = false;
			} else {
